...
describe('TestData', () => {
  it('testDataEncodingRestore', () => {
    ...
    inputText.append(' Wikipedia is the largest of the more than 290 Wikipedia encyclopedias. Overall, Wikipedia consists of more than 40 million articles in more than 250 different languages[15] and as of February 2014, ');
    ...
    const encodeRes: StringBuilder = data1.encode(new StringBuilder(), new ClassicEncodingSettings(false), false, 0);
    const data2: Data = Data.fromString(encodeRes.toString());
    const outputText: string = (data2.getData() as ByteBuffer).toUTF8();

    expect(inputText.toString()).toBe(outputText);
  });
});
