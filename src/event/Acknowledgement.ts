...
export default class Acknowledgement extends JObject {
  ...
  static __static_initializer_0() {
    ...
    Acknowledgement.FORMAT.addField(
      '<' + Acknowledgement.FIELD_AUTHOR + '><S><F=N><D=' + Cres.get().getString('author') + '>'
    );
    ...
    Acknowledgement.FORMAT.setNamingExpression(
      'print({}, "{' +
        Acknowledgement.FIELD_TIME +
        "} + ': ' + {" +
        Acknowledgement.FIELD_TEXT +
        "} + ' (' + {" +
        Acknowledgement.FIELD_AUTHOR +
        '} + \')\'", "; ")'
    );
