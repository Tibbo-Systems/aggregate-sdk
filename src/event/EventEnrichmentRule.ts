...
export default class EventEnrichmentRule extends AggreGateBean {
  ...
  static FIELD_EXPRESSION = 'expression';
  ...
  static __static_initializer_0() {
    ...
    const ff: FieldFormat<any> = FieldFormatFactory.create('<' + EventEnrichmentRule.FIELD_NAME + '><S><F=K><D=' + Cres.get().getString('name') + '>');
    ...
    EventEnrichmentRule.FORMAT.addField(FieldFormatFactory.create('<' + EventEnrichmentRule.FIELD_EXPRESSION + '><S><D=' + Cres.get().getString('expression') + '><E=' + FieldConstants.EDITOR_EXPRESSION + '>'));
  ...
  public getEnrichmentExpression(): Expression | null {
    if (this.enrichmentExpression == null) {
      this.enrichmentExpression = this.expression != null && this.expression.length > 0 ? new Expression(this.expression) : null;
    }
