...
export default class DataTableBuilding {
  ...
  public static createTableFormat(
    formatTable: DataTable,
    settings: ClassicEncodingSettings | null = null,
    ...
    const format: TableFormat = this.createTableFormatWithRecordsCount(
      minRecords,
      maxRecords,
      reorderable,
      fields,
    ...
    for (let binding of bindings) {
      format.addBinding(
        binding.getString(DataTableBuildingConstants.FIELD_BINDINGS_TARGET),
        binding.getString(DataTableBuildingConstants.FIELD_BINDINGS_EXPRESSION)
      );
    }
  ...
  public static createTableFormatWithRecordsCount(
    minRecords: number,
    maxRecords: number,
    reorderable: boolean,
    fields: DataTable,
    ...
    for (let fdata of fields) {
      let ff = FieldFormatFactory.createType(
        fdata.getString(DataTableBuildingConstants.FIELD_FIELDS_FORMAT_NAME),
        fdata.getString(DataTableBuildingConstants.FIELD_FIELDS_FORMAT_TYPE).charAt(0)
      );
      ...
      let key: boolean = fdata.getFormat().hasField(DataTableBuildingConstants.FIELD_FIELDS_FORMAT_KEY)
        ? fdata.getBoolean(DataTableBuildingConstants.FIELD_FIELDS_FORMAT_KEY)
        : false;
      ff.setKeyField(key);
      if (key) {
        hasKeys = true;
      ...
      if (selVals.getRecordCount() > 0) {
        ...
        for (let rec of selVals) {
          const val: string = rec.getString(DataTableBuildingConstants.FIELD_SELECTION_VALUES_VALUE);
          sv.set(
            val != null ? ff.valueFromString(val) : null,
            rec.getString(DataTableBuildingConstants.FIELD_SELECTION_VALUES_DESCRIPTION)
          );
        }
        ...
        if (!ff.isExtendableSelectionValues()) {
          if (!sv.has(ff.getDefaultValue())) {
            if (def != null || ff.isNullable()) {
              sv.set(
                def != null ? ff.valueFromString(def) : null,
                def != null ? def.toString() : Cres.get().getString('dtNullCell')
              );
            }
          }
        }
  ...
  public static formatToFieldsTable(
    tf: TableFormat,
    readOnly: boolean,
    settings: ClassicEncodingSettings | null = null,
    ignoreHiddenFields: boolean = true
  ): DataTable {
    ...
    for (let ff of tf) {
      ...
      if (ff.getValidators() != null) {
        for (let validator of ff.getValidators()) {
          validators.addRecordWith(
            (validator as AbstractFieldValidator<any>).getType(),
            (validator as AbstractFieldValidator<any>).encode()
          );
        }
      }
      ...
      let helpTxt: string | null = help;
      if (help != null) {
        //TODO may be broken
        const helpTxtPattern = new RegExp('/[p{Cntrl}[^p{Graph}]]/', 'a');
      ...
      rec.setValue(
        DataTableBuildingConstants.FIELD_FIELDS_FORMAT_DEFAULT_VALUE,
        ff.valueToString(ff.getDefaultValue(), settings == undefined ? undefined : settings)
      );
      ...
      try {
        ...
        const optionsTable: DataTable = EditorOptionsUtils.createEditorOptionsTable(
          type,
          editor,
          ff.getEditorOptions()
        );
  ...
  public static formatToTable(
    ...
    for (let binding of tf.getBindings()) {
      try {
        bindings
          .addRecord()
  ...
  static __static_initializer_0() {
    DataTableBuilding.SELECTION_VALUES_FORMAT.addField(
      '<' +
        DataTableBuildingConstants.FIELD_SELECTION_VALUES_VALUE +
        '><S><F=NK><D=' +
    ...
    DataTableBuilding.SELECTION_VALUES_FORMAT.addField(
      '<' +
        DataTableBuildingConstants.FIELD_SELECTION_VALUES_DESCRIPTION +
        '><S><D=' +
  ...
  static __static_initializer_1() {
    DataTableBuilding.BINDINGS_FORMAT.addField(
      '<' +
        DataTableBuildingConstants.FIELD_BINDINGS_TARGET +
        '><S><D=' +
        Cres.get().getString('target') +
        '><V=<L=1 ' +
        2147483647 +
        '>>'
    );
    DataTableBuilding.BINDINGS_FORMAT.addField(
      '<' +
        DataTableBuildingConstants.FIELD_BINDINGS_EXPRESSION +
        '><S><D=' +
  ...
  static __static_initializer_2() {
    let ff = FieldFormatFactory.create(
      '<' + DataTableBuildingConstants.FIELD_VALIDATORS_VALIDATOR + '><S><F=N><D=' + Cres.get().getString('type') + '>'
    );
    ff.setSelectionValues(DataTableUtils.getValidatorSelectionValues());
    DataTableBuilding.VALIDATORS_FORMAT.addField(ff);
    DataTableBuilding.VALIDATORS_FORMAT.addField(
      '<' + DataTableBuildingConstants.FIELD_VALIDATORS_OPTIONS + '><S><D=' + Cres.get().getString('options') + '>'
    );
  }
  ...
  static __static_initializer_3() {
    DataTableBuilding.EDITOR_OPTIONS_SIMPLE_FORMAT.addField(
      FieldFormatFactory.createWith(
        DataTableBuildingConstants.FIELD_EDITOR_OPTIONS_SIMPLE_FORMAT_OPTIONS,
        FieldConstants.STRING_FIELD,
        Cres.get().getString('dtEditorOptions')
      )
    );
  }
  ...
  static __static_initializer_4() {
    ...
    DataTableBuilding.FIELDS_FORMAT.addField(
      FieldFormatFactory.create('<' + DataTableBuildingConstants.FIELD_FIELDS_FORMAT_OLDNAME + '><S><F=H>')
    );
    let ff = FieldFormatFactory.create(
      '<' + DataTableBuildingConstants.FIELD_FIELDS_FORMAT_NAME + '><S><F=K><D=' + Cres.get().getString('name') + '>'
    );
    ...
    ff = FieldFormatFactory.create(
      '<' +
        DataTableBuildingConstants.FIELD_FIELDS_FORMAT_DESCRIPTION +
        '><S><F=N><D=' +
        Cres.get().getString('description') +
        '>'
    );
    ...
    DataTableBuilding.FIELDS_FORMAT.addField(
      FieldFormatFactory.create(
        '<' +
          DataTableBuildingConstants.FIELD_FIELDS_FORMAT_READONLY +
          '><B><D=' +
          Cres.get().getString('dtReadOnly') +
          '>'
      )
    );
    DataTableBuilding.FIELDS_FORMAT.addField(
      FieldFormatFactory.create(
        '<' +
          DataTableBuildingConstants.FIELD_FIELDS_FORMAT_NULLABLE +
          '><B><D=' +
          Cres.get().getString('dtNullable') +
          '>'
      )
    );
    DataTableBuilding.FIELDS_FORMAT.addField(
      FieldFormatFactory.create(
        '<' + DataTableBuildingConstants.FIELD_FIELDS_FORMAT_KEY + '><B><D=' + Cres.get().getString('dtKeyField') + '>'
      )
    );
    ff = FieldFormatFactory.create(
      '<' +
        ...
        '><T><D=' +
        Cres.get().getString('dtSelectionValues') +
        '>'
    );
    ...
    DataTableBuilding.FIELDS_FORMAT.addField(
      FieldFormatFactory.create(
        '<' +
          DataTableBuildingConstants.FIELD_FIELDS_FORMAT_EXTSELVALS +
          '><B><D=' +
          Cres.get().getString('dtExtendableSelVals') +
          '>'
      )
    );
    ff = FieldFormatFactory.create(
      '<' + DataTableBuildingConstants.FIELD_FIELDS_FORMAT_HIDDEN + '><B><D=' + Cres.get().getString('hidden') + '>'
    );
    ff.setAdvanced(true);
    DataTableBuilding.FIELDS_FORMAT.addField(ff);
    ff = FieldFormatFactory.create(
      '<' + DataTableBuildingConstants.FIELD_FIELDS_FORMAT_INLINE + '><B><D=' + Cres.get().getString('inline') + '>'
    );
    ff.setAdvanced(true);
    DataTableBuilding.FIELDS_FORMAT.addField(ff);
    ff = FieldFormatFactory.create(
      '<' + DataTableBuildingConstants.FIELD_FIELDS_FORMAT_HELP + '><S><F=N><D=' + Cres.get().getString('help') + '>'
    );
    ff.getValidators().push(ValidatorHelper.DESCRIPTION_SYNTAX_VALIDATOR);
    ...
    ff = FieldFormatFactory.create(
      '<' +
        ...
        '><S><F=N><D=' +
        Cres.get().getString('dtEditorViewer') +
        '>'
    );
    ...
    exp =
      ...
      Contexts.CTX_UTILITIES +
      ':' +
      UtilitiesContextConstants.F_EDITORS +
      "('{" +
      DataTableBuildingConstants.FIELD_FIELDS_FORMAT_TYPE +
      "}')}";
    DataTableBuilding.FIELDS_FORMAT.addBinding(ref, exp);
    ff = FieldFormatFactory.create(
      '<' +
        DataTableBuildingConstants.FIELD_FIELDS_FORMAT_VALIDATORS +
        '><T><F=N><D=' +
        Cres.get().getString('dtValidators') +
        '>'
    );
    ff.setDefault(DataTableFactory.of(DataTableBuilding.VALIDATORS_FORMAT));
    DataTableBuilding.FIELDS_FORMAT.addField(ff);
    ff = FieldFormatFactory.create(
      '<' + DataTableBuildingConstants.FIELD_FIELDS_FORMAT_GROUP + '><S><F=N><D=' + Cres.get().getString('group') + '>'
    );
    DataTableBuilding.FIELDS_FORMAT.addField(ff);
  }
  ...
  static __static_initializer_5() {
    let ff = FieldFormatFactory.create(
      '<' +
        DataTableBuildingConstants.FIELD_TABLE_FORMAT_MIN_RECORDS +
        '><I><A=0><D=' +
        Cres.get().getString('dtMinRecords') +
        '>'
    );
    ...
    ff = FieldFormatFactory.create(
      '<' +
        DataTableBuildingConstants.FIELD_TABLE_FORMAT_MAX_RECORDS +
        '><I><F=E><A=' +
        2147483647 +
        '><D=' +
        Cres.get().getString('dtMaxRecords') +
        '><S=<' +
        Cres.get().getString('dtNotLimited') +
        '=' +
        2147483647 +
        '>>'
    );
    ...
    DataTableBuilding.TABLE_FORMAT.addField(ff);
    ff = FieldFormatFactory.create(
      '<' + DataTableBuildingConstants.FIELD_TABLE_FORMAT_FIELDS + '><T><D=' + Cres.get().getString('fields') + '>'
    );
    ff.setDefault(DataTableFactory.of(DataTableBuilding.FIELDS_FORMAT));
    DataTableBuilding.TABLE_FORMAT.addField(ff);
    ff = FieldFormatFactory.create(
      '<' +
        DataTableBuildingConstants.FIELD_TABLE_FORMAT_REORDERABLE +
        '><B><D=' +
        Cres.get().getString('dtReorderable') +
        '>'
    );
    DataTableBuilding.TABLE_FORMAT.addField(ff);
    ff = FieldFormatFactory.create(
      '<' +
        DataTableBuildingConstants.FIELD_TABLE_FORMAT_UNRESIZABLE +
        '><B><D=' +
        Cres.get().getString('dtUnresizable') +
        '>'
    );
    DataTableBuilding.TABLE_FORMAT.addField(ff);
    ff = FieldFormatFactory.create(
      '<' + DataTableBuildingConstants.FIELD_TABLE_FORMAT_BINDINGS + '><T><D=' + Cres.get().getString('bindings') + '>'
    );
    ...
    ff = FieldFormatFactory.create(
      '<' +
        ...
        '><S><F=R><D=' +
        Cres.get().getString('dtEncodedFormat') +
        '><H=' +
        Cres.get().getString('dtEncodedFormatHelp') +
        '><E=' +
    ...
    ff = FieldFormatFactory.create(
      '<' +
        DataTableBuildingConstants.FIELD_TABLE_FORMAT_NAMING_EXPRESSION +
        '><S><F=N><D=' +
        Cres.get().getString('namingExpression') +
        '><E=' +
        FieldConstants.EDITOR_EXPRESSION +
        '>'
    );
    ...
    DataTableBuilding.TABLE_FORMAT.setNamingExpression(
      Functions.PRINT +
        '({' +
        DataTableBuildingConstants.FIELD_TABLE_FORMAT_FIELDS +
        "}, '{" +
        DataTableBuildingConstants.FIELD_FIELDS_FORMAT_NAME +
        "}', ', ')"
    );
  }
