  ...
  execute(evaluator: Evaluator, environment: EvaluationEnvironment, parameters: Array<any>): any {
    ...
    try {
      if (parameters.length >= 2 && parameters[1] != null && evaluator.getDefaultResolver().getContextManager() != null) {
        const defaultContext = evaluator.getDefaultResolver().getContextManager()?.get(parameters[1].toString(), evaluator.getDefaultResolver().getCallerController());
      ...
      if (parameters.length >= 4) {
        ...
        if (number != null) {
          evaluator.getDefaultResolver().setDefaultRow(number);
        }
      }
      return evaluator.evaluate(new Expression(parameters[0].toString()));
    } finally {
      if (defaultRowBackup != null) {
