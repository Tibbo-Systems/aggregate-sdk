...
export default class SetVariableFunction extends AbstractFunction {
  ...
  async asyncExecute(evaluator: Evaluator, environment: EvaluationEnvironment, parameters: Array<any>): Promise<any> {
    ...
    try {
      ...
      const valueTable = paramsInput.length == 1 && paramsInput[0] instanceof DataTable ? paramsInput[0] : await DataTableConstruction.constructTable(paramsInput, vd.getFormat(), evaluator, environment);
