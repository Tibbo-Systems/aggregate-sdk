...
export default class FireEventFunction extends AbstractFunction {
  ...
  async asyncExecute(evaluator: Evaluator, environment: EvaluationEnvironment, parameters: Array<any>): Promise<any> {
    ...
    try {
      ...
      const data = input.length == 1 && input[0] instanceof DataTable ? input[0] : await DataTableConstruction.constructTable(input, ed.getFormat(), evaluator, environment);

      const ev = await con.fireEvent(ed.getName(), data, level != null ? level : ed.getLevel(), evaluator.getDefaultResolver().getCallerController());
