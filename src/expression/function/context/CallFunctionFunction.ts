...
export default class CallFunctionFunction extends AbstractFunction {
  ...
  async asyncExecute(evaluator: Evaluator, environment: EvaluationEnvironment, parameters: Array<any>): Promise<any> {
    ...
    try {
      ...
      const inputTable = await this.constructInputTable(input, fd, evaluator, environment);
  ...
  private async constructInputTable(input: Array<any>, fd: FunctionDefinition, evaluator: Evaluator, environment: EvaluationEnvironment) {
    ...
    return await DataTableConstruction.constructTable(input, fd != null ? fd.getInputFormat() : null, evaluator, environment);
  }
}
