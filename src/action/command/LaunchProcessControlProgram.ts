...
export default class LaunchProcessControlProgram extends LaunchWidget {
  public static readonly FIFT_DEBUG_PROGRAM: TableFormat = new TableFormat(1, 1, '<' + ProcessControlContextConstants.FIF_DEBUG_PROGRAM_PARAMETERS + '><I><A=1>');

  public static readonly FIFT_BREAKPOINT: TableFormat = new TableFormat();

  static __static_initializer_0() {
    LaunchProcessControlProgram.FIFT_BREAKPOINT.addField(FieldFormatFactory.createType(ProcessControlContextConstants.FIF_BREAKPOINT_POSITION, FieldConstants.STRING_FIELD));
    LaunchProcessControlProgram.FIFT_BREAKPOINT.addField(FieldFormatFactory.createType(ProcessControlContextConstants.FIF_BREAKPOINT_POSITION_LINE, FieldConstants.INTEGER_FIELD).setDefault(-1));
  }
  ...
  public constructor(title?: string, widgetContext?: string, defaultContext?: string, template?: string) {
    ...
    this.setType(ActionUtilsConstants.CMD_LAUNCH_PROCESS_CONTROL_PROGRAM);
  }
